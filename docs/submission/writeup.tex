...
\title{\vspace{-1.5cm}\textbf{An Adversarial Framework for Payment Security}\\[0.25cm]
\large Red-teaming fraud detection and agentic payment systems}
...
\begin{itemize}
  ...
  \item \textbf{Attack success does not fall.} It is \AsrFirst\% at round~0 and \AsrLast\% at
        the final round. What moved is cost: mean features touched \LzeroFirst{} $\to$
        \LzeroLast{} (+\LzeroRise\%) and median queries \QueriesFirst{} $\to$ \QueriesLast{}
        (+\QueriesRise\%).
  ...
  \item \textbf{The defence does detect the attacks it was trained on the shape of.} On
        ...
        \AdvDeclinesBefore{} to \AdvDeclinesAfter{} per 100k.
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{Attack success is flat at \AsrLast\% across every round. The attacker's cost is
  not: it touches more features and spends more queries each round to find an evasion that
  still works. A defence that raises the price of an attack without preventing it is a real
  outcome, and reporting it as a fall in success rate would have been the easier story.}
  \label{fig:coevolution}
\end{figure}

\noindent
The detector for this loop is P1's published round~0 on a temporal split
(PR-AUC \PrAucTemporal{}, $n_{\text{train}}=\NTrainTemporal$). The sweeps below use a
stratified split at a larger training size and are a \emph{different experiment}; their
PR-AUC is not comparable to this one and the two never appear in the same claim.
...
\begin{figure}[H]
  ...
  the only honest one. Real-fraud recall holds and legitimate declines fall.}
  \label{fig:advdet}
\end{figure}
